...
function App() {
  ...
  return <>
    <header className="site-header"><nav className="nav shell"><a className="brand" href="#top" aria-label="PivotVault home"><img className="brand-flower" src="/pivotvault-logo.svg" alt="" width={30} height={30} /><span>PivotVault<small>Startup Failure Intelligence</small></span></a><div className="nav-links">{navItems.map(([name, target]) => <a key={name} href={`#${target}`}>{name}</a>)}</div><div className="nav-actions"><a href="#demo" className="signin">Sign In</a><button className="button dark small" onClick={goToScan}>Explore Archive <ArrowUpRight size={15}/></button></div><button className="menu-toggle" aria-expanded={menuOpen} aria-controls="mobile-menu" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X /> : <Menu />}<span className="sr-only">Toggle navigation</span></button></nav>
